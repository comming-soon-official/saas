...
const Index = (props) => {
  ...
  const table1Html = tabledata1.slice(0, size1.value).map((row) => (
    <tr>
  ...
  const table2Html = tabledata2.slice(0, size2.value).map((row) => (
    <tr>
      {Object.keys(row).map((col) => (
        <td>{row[col]}</td>
      ))}
  ...
  const toggle = (e) => {
    if (e.target.id == "table1") {
      size1.value === 3
        ? setSize1({ value: tabledata1.length - 1, msg: "Collapse Table" })
        : setSize1({ value: 3, msg: "Expand Table" });
    } else {
      size2.value === 3
        ? setSize2({ value: tabledata2.length - 1, msg: "Collapse Table" })
        : setSize2({ value: 3, msg: "Expand Table" });
    }
  };
